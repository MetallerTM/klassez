...
class Spectrum_1D:
    ...
    F : fit.Voigt_Fit object
        Used for deconvolution. See :class:`klassez.fit.Voigt_fit`
    baseline : 1darray
    ...
    def rpbc(self, **rpbc_kws):
        """
        Computes the phase angles and the baseline using :func:`klassez.processing.rpbc` on ``self.S``.
    ...
    def to_vf(self, filename=None, Hs=None, fvf=True):
        ...
        To do this, it reads the peak parameters saved in ``self.acqus``.
...
class pSpectrum_1D(Spectrum_1D):
    ...
    .. seealso::
        ...
        :class:`klassez.Spectra.Spectrum_1D`
...
class Spectrum_2D:
    ...
    def eae(self):
        """
        Calls :func:`klassez.processing.eae` to shuffle the data and make a States-like FID.
    ...
    def xf2(self):
        ...
        Calls :func:`klassez.processing.fp` using ``self.procs[keys][1]``
        The result is stored in ``self.S``, then ``self.rr`` and ``self.ii`` are written.
        ``self.freq_f1`` and ``self.ppm_f1`` are assigned with the indexes of the transients.
    ...
    def splitcomb(self, J=53.8):
        ...
        .. seealso::
            ...
            :func:`klassez.processing.splitcomb`
        """
    ...
    def xf1(self):
        ...
        The result is stored in ``self.S``, then ``self.rr`` and ``self.ii`` are written.
        ``freq_f2`` and ``ppm_f2`` are assigned with the indexes of the indirect evolutions.
        """
        # Transpose
    ...
    def inv_process(self):
        ...
        Performs the inverse processing of the spectrum according to the given parameters.
        Overwrites the ``S`` attribute!!
        Calls :func:`klassez.processing.inv_xfb`.
        """
    ...
    def adjph(self, p01=None, p11=None, pv1=None, p02=None, p12=None, pv2=None, update=True):
        ...
        The non-interactive workflow is to apply :func:`klassez.processing.ps` on F2, transpose according to ``FnMODE``, apply :func:`klassez.processing.ps` on F1, transpose back.
...
class pSpectrum_2D(Spectrum_2D):
    ...
    acqus : dict
        Dictionary of acqusition parameters
    ngdic : dict
        Generated by :func:`nmrglue.bruker.read`, contains all the information on the spectrometer and on the spectrum.
    procs : dict
        Dictionary of processing parameters
    S : 2darray
        Complex (or hypercomplex, depending on ``FnMODE``) spectrum
...
class Pseudo_2D(Spectrum_2D):
    ...
    def cal(self, offset=None, isHz=False, update=True, from_procs=True, *, ref=None):
        ...
        .. seealso::
            ...
            :func:`klassez.processing.calibrate`
        """
        def _calibrate(ppm, trace, SFO1, ref=None):
            """ Main function that calls the real calibration """
            offppm = processing.calibration(ppm, trace, ref=ref)
...
class DOSY_SSel(DOSY_T1):
    ...
    def __init__(self, in_file):
        ...
        # Load the F2 list
        #   It should be the only file in lists/vd/ but we do not know its name
        fqlistpath = next((self.datadir / 'lists' / 'f1').iterdir())
        #   Print a notification
        print(f'Found {fqlistpath} to be imported as FQLIST', c='tab:blue')
        #   Actual loading and storage in an attribute
        fqlist = np.loadtxt(fqlistpath)
        self.x_f2 = fqlist
        self.label_f2 = 'Freq /Hz'
