...
Lightened the *__init__* method of the *Spectra* classes by using new wrappers in *misc*.
These function use a ``match/case`` structure, therefore the minimum required version of python was increased to 3.10.
The default **procs** dictionary is generated by a new internal "private" method *_default_procs*.

The *Spectra* classes now unpack the real and imaginary part(s) internally every time the attribute **self.S** is created or overwritten.
All the input/output and file handling is now done using *pathlib.Path* objects instead of *os*.
...
## New functions:
- *misc.get_datadir_filename*
- *misc.get_extent*
- *misc.read_fid_acqus_1D*
- *misc.read_fid_acqus_2D*
- *figures.plot_1D*
- *figures.plot_2D*
- *figures.plot_p3D*
- *fit.fit_dosy_multi*
